...
A: 
    while TRUE do 
        await state = "closing" /\ reciveReq # <<>>;
        
        if reciveReq # CORRUPT_DATA then
            if reciveReq[1] = "FIN" then 
                state := "FIN-ACK"
            end if;
        end if;
        reciveReq := <<>>;
...
A: 
    while TRUE do 
        await state = "FIN-ACK" /\ reciveReq # <<>>;
        
        if reciveReq # CORRUPT_DATA then
            if reciveReq[1] = "ACK" then 
                state := "closed";
            end if;
        end if;
        reciveReq := <<>>;
...
FIN == /\ state = "closing" /\ reciveReq # <<>>
       /\ IF reciveReq # CORRUPT_DATA
             THEN /\ IF reciveReq[1] = "FIN"
                        THEN /\ state' = "FIN-ACK"
                        ELSE /\ TRUE
                             /\ state' = state
             ELSE /\ TRUE
...
FINACK == /\ state = "FIN-ACK" /\ reciveReq # <<>>
          /\ IF reciveReq # CORRUPT_DATA
                THEN /\ IF reciveReq[1] = "ACK"
                           THEN /\ state' = "closed"
                           ELSE /\ TRUE
                                /\ state' = state
                ELSE /\ TRUE
...
Properties == \A x \in {"opening", "SYN_ACK_RECIVED", "open","closing", "closed", "FIN-ACK" }: <>( state = x )
...
\* Last modified Wed Jun 12 23:53:57 NZST 2019 by sdmsi
